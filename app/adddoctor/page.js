...
const AddDoctorPage = () => {
  ...
  const changeSignatureHandler = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    try {
      ...
      const res = await fetch("/api/auth/adddoctor", {
        ...
        body: JSON.stringify({
          ...
          specialization: formData.specialization,
          imageUrl: formData.image,
          signature: formData.signature, // new field
          doc_id: doc_id
        }),
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-white p-6">
      <div className="bg-white shadow-md rounded-lg p-8 max-w-4xl w-full">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6" onSubmit={handleSubmit}>
          ...
          {/* Signature */}
          <div className="flex flex-col">
            <label className="mb-2 font-medium text-gray-700">Signature</label>
            <input type="file" name="signature" accept="image/*" onChange={changeSignatureHandler} />
            {formData.signature && <p className="mt-2 text-sm text-gray-600">Signature uploaded</p>}
          </div>

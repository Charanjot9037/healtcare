...
//   const [responses, setResponses] = useState([]); // {type: 'user'|'bot', text: html}
...
//       <div className="h-[60vh] overflow-y-auto border rounded p-3 space-y-3 bg-gray-50">
...
// // small helper to avoid accidental broken HTML when adding user text
  ...
  const sendRequest = async (payload, userBubbleHtml = null) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResponses((p) => [...p, { type: "bot", text: data.html || "<p>No response</p>" }]);
        if (data.pastEntries) setPastEntries(data.pastEntries.reverse());
      }
    } catch (err) {
      setResponses((p) => [...p, { type: "bot", text: `<p style="color:red">Network error: ${err.message}</p>` }]);
    } finally {
